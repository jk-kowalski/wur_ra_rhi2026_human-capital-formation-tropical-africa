...
-- Project      : RHI 2026 – IPUMS Africa (Benin) Preprocessing
-- Script name  : 1.01_benin.sql
-- Last updated : 16/04/2026
-- Purpose      : Preprocess IPUMS census data (Benin) for statistical analysis and spatial integration.
...
CREATE TABLE public.extract_1benin (
  "COUNTRY"         text,
  "YEAR"            text,
  "SAMPLE"          text,
  "SERIAL"          text,
...
-- Backup
CREATE TABLE extract_1benin_backup AS
SELECT * FROM extract_1benin;

-- Master table for further analysis
CREATE TABLE master_1benin AS
SELECT * FROM extract_1benin;
...
ALTER TABLE public.master_1benin
ADD COLUMN IF NOT EXISTS birth_province integer;

UPDATE public.master_1benin
SET birth_province = CASE
    WHEN CAST("BPL1_BJ" AS integer) BETWEEN 204001 AND 204012 THEN CAST("BPL1_BJ" AS integer)
    WHEN CAST("BPL1_BJ" AS integer) IN (204097, 204098) THEN NULL
    ELSE NULL
END;
-- ----------------------------------------------------------
...
UPDATE public.master_1benin
SET "FOREIGN" = CASE
    WHEN CAST("BPL1_BJ" AS integer) = 204097 THEN 1
    WHEN CAST("BPL1_BJ" AS integer) = 204098 THEN NULL
    ELSE 0
END;
...
DELETE FROM public.master_1benin
...
DROP TABLE IF EXISTS public.edu_residence_1benin;

CREATE TABLE public.edu_residence_1benin AS
SELECT
    cohort,
    "GEO1_BJ",
    ...
    -- counts
    COUNT(primary_educ) AS nc_benin

FROM public.master_1benin
WHERE cohort NOT IN (1905, 1985)
GROUP BY cohort, "GEO1_BJ";


-- Join
DROP TABLE IF EXISTS public.geom_edu_residence_1benin;

CREATE TABLE public.geom_edu_residence_1benin AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
...
LEFT JOIN public.edu_residence_1benin e
    ON CAST(s."GEOLEVEL1" AS integer) = CAST(e."GEO1_BJ" AS integer)
WHERE CAST(s."GEOLEVEL1" AS integer) BETWEEN 204001 AND 204012;

-- ----------------------------------------------------------
-- 4.2 Summary by birthplace region
-- ----------------------------------------------------------

DROP TABLE IF EXISTS public.edu_birthplace_1benin;

CREATE TABLE public.edu_birthplace_1benin AS
...
-- join
DROP TABLE IF EXISTS public.geom_edu_birthplace_1benin;

CREATE TABLE public.geom_edu_birthplace_1benin AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
    ...
    e.nc_benin
FROM public.world_geolev1_2021 s
LEFT JOIN public.edu_birthplace_1benin e
