...
WITH recoded_1998 AS (
    SELECT
        s.*,
        CASE
            WHEN TRIM(s."IPUM1998") IN ('001', '013', '015', '016') THEN 384001
            -- add other recodes here
            ELSE CAST(TRIM(s."IPUM1998") AS integer) + 384000
        END AS geolevel1_new
    FROM public.geo1_ci1998 s
),
geom_dissolved AS (
    SELECT
        MIN(id)::integer AS id,
        ST_Multi(ST_Union(geom)) AS geom,
        geolevel1_new AS "GEOLEVEL1"
    FROM recoded_1998
    GROUP BY geolevel1_new
),
admin_lookup AS (
    SELECT DISTINCT
        CASE
...
SELECT
    d.id,
    d.geom,
    'Cote d''Ivoire'::text AS "CNTRY_NAME",
    a."ADMIN_NAME",
    '384'::text AS "CNTRY_CODE",
    d."GEOLEVEL1",
    d."GEOLEVEL1"::text AS "BPL_CODE"
FROM geom_dissolved d
LEFT JOIN admin_lookup a
    ON d."GEOLEVEL1" = a."GEOLEVEL1";
