...
-- Project      : RHI 2026 – IPUMS Africa (Guinea) Preprocessing
-- Script name  : 1.07_guinea.sql
-- Last updated : 16/04/2026
-- Purpose      : Preprocess IPUMS census data (Guinea) for statistical analysis and spatial integration.
...
CREATE TABLE public.extract_7guinea (
  "COUNTRY"         text,
  "YEAR"            text,
  "SAMPLE"          text,
  "SERIAL"          text,
...
-- Backup
CREATE TABLE extract_7guinea_backup AS
SELECT * FROM extract_7guinea;

-- Master table for further analysis
CREATE TABLE master_7guinea AS
SELECT * FROM extract_7guinea;
...
ALTER TABLE public.master_7guinea
ADD COLUMN IF NOT EXISTS birth_province integer;

-- Deviation from other scripts, BPLNG (prefacture of birth) follows a different format from geolev1 in world file used for spatial join. Recoding is a must. 
UPDATE public.master_7guinea
SET birth_province = CASE
    -- Boké region
    WHEN CAST("BPLGN" AS integer) = 11 THEN 324011  -- Boffa
    WHEN CAST("BPLGN" AS integer) = 12 THEN 324012  -- Boke
    WHEN CAST("BPLGN" AS integer) = 13 THEN 324013  -- Fria
    WHEN CAST("BPLGN" AS integer) = 14 THEN 324014  -- Gaoual
    WHEN CAST("BPLGN" AS integer) = 15 THEN 324015  -- Koundara

    -- Conakry
    WHEN CAST("BPLGN" AS integer) = 81 THEN 324021  -- Conakry

    -- Faranah region
    WHEN CAST("BPLGN" AS integer) = 21 THEN 324031  -- Dabola
    WHEN CAST("BPLGN" AS integer) = 22 THEN 324032  -- Dinguiraye
    WHEN CAST("BPLGN" AS integer) = 23 THEN 324033  -- Faranah
    WHEN CAST("BPLGN" AS integer) = 24 THEN 324034  -- Kissidougou

    -- Kankan region
    WHEN CAST("BPLGN" AS integer) = 31 THEN 324041  -- Kankan
    WHEN CAST("BPLGN" AS integer) = 32 THEN 324042  -- Kerouane
    WHEN CAST("BPLGN" AS integer) = 33 THEN 324043  -- Kouroussa
    WHEN CAST("BPLGN" AS integer) = 34 THEN 324044  -- Mandiana
    WHEN CAST("BPLGN" AS integer) = 35 THEN 324045  -- Siguiri

    -- Kindia region
    WHEN CAST("BPLGN" AS integer) IN (41, 42) THEN 324051  -- Coyah, Dubreka
    WHEN CAST("BPLGN" AS integer) = 43 THEN 324053       -- Forecariah
    WHEN CAST("BPLGN" AS integer) = 44 THEN 324054       -- Kindia
    WHEN CAST("BPLGN" AS integer) = 45 THEN 324055       -- Telimele

    -- Labé region
    WHEN CAST("BPLGN" AS integer) = 51 THEN 324061  -- Koubia
    WHEN CAST("BPLGN" AS integer) = 52 THEN 324062  -- Labe
    WHEN CAST("BPLGN" AS integer) = 53 THEN 324063  -- Lelouma
    WHEN CAST("BPLGN" AS integer) = 54 THEN 324064  -- Mali
    WHEN CAST("BPLGN" AS integer) = 55 THEN 324065  -- Tougue

    -- Mamou region
    WHEN CAST("BPLGN" AS integer) = 61 THEN 324071  -- Dalaba
    WHEN CAST("BPLGN" AS integer) = 62 THEN 324072  -- Mamou
    WHEN CAST("BPLGN" AS integer) = 63 THEN 324073  -- Pita

    -- Nzérékoré region
    WHEN CAST("BPLGN" AS integer) = 71 THEN 324081  -- Beyla
    WHEN CAST("BPLGN" AS integer) = 72 THEN 324082  -- Gueckedou
    WHEN CAST("BPLGN" AS integer) = 73 THEN 324083  -- Lola
    WHEN CAST("BPLGN" AS integer) = 74 THEN 324084  -- Macenta
    WHEN CAST("BPLGN" AS integer) = 75 THEN 324085  -- Nzerekore
    WHEN CAST("BPLGN" AS integer) = 76 THEN 324086  -- Yomou

    -- Unknown / foreign
    WHEN CAST("BPLGN" AS integer) IN (97, 98, 99) THEN NULL
    ELSE NULL
END;
-- ----------------------------------------------------------
...
UPDATE public.master_7guinea
SET "FOREIGN" = CASE
    WHEN CAST("BPLGN" AS integer) = 98 THEN 1
    WHEN CAST("BPLGN" AS integer) IN (97, 99) THEN NULL
    ELSE 0
END;
...
DELETE FROM public.master_7guinea
...
DROP TABLE IF EXISTS public.edu_residence_7guinea;

CREATE TABLE public.edu_residence_7guinea AS
SELECT
    cohort,
    "GEO1_GN",
    ...
    -- counts
    COUNT(primary_educ) AS nc_guinea

FROM public.master_7guinea
WHERE cohort NOT IN (1905, 1985)
GROUP BY cohort, "GEO1_GN";


-- Join
DROP TABLE IF EXISTS public.geom_edu_residence_7guinea;

CREATE TABLE public.geom_edu_residence_7guinea AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
...
LEFT JOIN public.edu_residence_7guinea e
    ON CAST(s."GEOLEVEL1" AS integer) = CAST(e."GEO1_GN" AS integer)
WHERE CAST(s."GEOLEVEL1" AS integer) IN (
    324011, 324012, 324013, 324014, 324015,
    324021,
    324031, 324032, 324033, 324034,
    324041, 324042, 324043, 324044, 324045,
    324051, 324053, 324054, 324055,
    324061, 324062, 324063, 324064, 324065,
    324071, 324072, 324073,
    324081, 324082, 324083, 324084, 324085, 324086
);
...
DROP TABLE IF EXISTS public.edu_birthplace_7guinea;

CREATE TABLE public.edu_birthplace_7guinea AS
...
-- join
DROP TABLE IF EXISTS public.geom_edu_birthplace_7guinea;

CREATE TABLE public.geom_edu_birthplace_7guinea AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
    ...
    e.nc_guinea
FROM public.world_geolev1_2021 s
LEFT JOIN public.edu_birthplace_7guinea e
