...
-- Project      : RHI 2026 – IPUMS Africa (Malawi) Preprocessing
-- Script name  : 1.09_malawi.sql
-- Last updated : 16/04/2026
-- Purpose      : Preprocess IPUMS census data (Malawi) for statistical analysis and spatial integration.
...
CREATE TABLE public.extract_9malawi (
  "COUNTRY"         text,
  "YEAR"            text,
  "SAMPLE"          text,
  "SERIAL"          text,
...
-- Backup
CREATE TABLE extract_9malawi_backup AS
SELECT * FROM extract_9malawi;

-- Master table for further analysis
CREATE TABLE master_9malawi AS
SELECT * FROM extract_9malawi;
...
ALTER TABLE public.master_9malawi
ADD COLUMN IF NOT EXISTS birth_province integer;

UPDATE public.master_9malawi
SET birth_province = CASE
    WHEN CAST("BPL1_MW" AS integer) BETWEEN 454101 AND 454312 
        THEN CAST("BPL1_MW" AS integer)
    WHEN CAST("BPL1_MW" AS integer) IN (454997, 454998) 
        THEN NULL
    ELSE NULL
END;
-- ----------------------------------------------------------
...
UPDATE public.master_9malawi
SET "FOREIGN" = CASE
    WHEN CAST("BPL1_MW" AS integer) = 454997 THEN 1
    WHEN CAST("BPL1_MW" AS integer) = 454998 THEN NULL
    ELSE 0
END;
...
DELETE FROM public.master_9malawi
...
DROP TABLE IF EXISTS public.edu_residence_9malawi;

CREATE TABLE public.edu_residence_9malawi AS
SELECT
    cohort,
    "GEO1_MW",
    ...
    -- counts
    COUNT(primary_educ) AS nc_malawi

FROM public.master_9malawi
WHERE cohort NOT IN (1905, 1985)
GROUP BY cohort, "GEO1_MW";


-- Join
DROP TABLE IF EXISTS public.geom_edu_residence_9malawi;

CREATE TABLE public.geom_edu_residence_9malawi AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
...
LEFT JOIN public.edu_residence_9malawi e
    ON CAST(s."GEOLEVEL1" AS integer) = CAST(e."GEO1_MW" AS integer)
WHERE CAST(s."GEOLEVEL1" AS integer) IN (
    454101, 454102, 454103, 454104, 454105,
    454201, 454202, 454203, 454204, 454205,
    454206, 454207, 454208, 454209,
    454301, 454302, 454303, 454304, 454305,
    454306, 454307, 454308, 454309, 454310,
    454311, 454312
);
...
DROP TABLE IF EXISTS public.edu_birthplace_9malawi;

CREATE TABLE public.edu_birthplace_9malawi AS
...
-- join
DROP TABLE IF EXISTS public.geom_edu_birthplace_9malawi;

CREATE TABLE public.geom_edu_birthplace_9malawi AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
    ...
    e.nc_malawi
FROM public.world_geolev1_2021 s
LEFT JOIN public.edu_birthplace_9malawi e
