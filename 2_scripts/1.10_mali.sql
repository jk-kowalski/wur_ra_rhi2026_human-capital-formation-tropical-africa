...
-- Project      : RHI 2026 – IPUMS Africa (Mali) Preprocessing
-- Script name  : 1.10_mali.sql
-- Last updated : 16/04/2026
-- Purpose      : Preprocess IPUMS census data (Mali) for statistical analysis and spatial integration.
...
-- Description:
  ...
  -- Source data:
    -- IPUMS International: SMali Census Microdata (2009, 1998, 1987)
...
CREATE TABLE public.extract_10mali (
  "COUNTRY"         text,
  "YEAR"            text,
  "SAMPLE"          text,
  "SERIAL"          text,
...
-- Backup
CREATE TABLE extract_10mali_backup AS
SELECT * FROM extract_10mali;

-- Master table for further analysis
CREATE TABLE master_10mali AS
SELECT * FROM extract_10mali;
...
ALTER TABLE public.master_10mali
ADD COLUMN IF NOT EXISTS birth_province integer;

-- BPLML exists on lvl2 only, aggregation needed. To do: re-check
UPDATE public.master_10mali
SET birth_province = CASE

    -- Kayes
    WHEN CAST("BPLML" AS integer) BETWEEN 10 AND 17 THEN 466001

    -- Koulikoro
    WHEN CAST("BPLML" AS integer) BETWEEN 20 AND 27 THEN 466002

    -- Sikasso
    WHEN CAST("BPLML" AS integer) BETWEEN 30 AND 37 THEN 466003

    -- Ségou
    WHEN CAST("BPLML" AS integer) BETWEEN 40 AND 47 THEN 466004

    -- Mopti
    WHEN CAST("BPLML" AS integer) BETWEEN 50 AND 58 THEN 466005

    -- Tombouctou
    WHEN CAST("BPLML" AS integer) BETWEEN 60 AND 65 THEN 466006

    -- Gao + Kidal
    WHEN CAST("BPLML" AS integer) BETWEEN 70 AND 84 THEN 466007

    -- Bamako
    WHEN CAST("BPLML" AS integer) BETWEEN 90 AND 97 THEN 466009

    -- Unspecified / foreign / unknown
    WHEN CAST("BPLML" AS integer) IN (98, 99, 998) THEN NULL
...
UPDATE public.master_10mali
SET "FOREIGN" = CASE
    WHEN CAST("BPLML" AS integer) = 99 THEN 1
    WHEN CAST("BPLML" AS integer) IN (98, 998) THEN NULL
    ELSE 0
END;
...
DELETE FROM public.master_10mali
...
DROP TABLE IF EXISTS public.edu_residence_10mali;

CREATE TABLE public.edu_residence_10mali AS
SELECT
    cohort,
    "GEO1_ML",
    ...
    -- counts
    COUNT(primary_educ) AS nc_mali

FROM public.master_10mali
WHERE cohort NOT IN (1905, 1985)
GROUP BY cohort, "GEO1_ML";

-- Join
DROP TABLE IF EXISTS public.geom_edu_residence_10mali;

CREATE TABLE public.geom_edu_residence_10mali AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
...
LEFT JOIN public.edu_residence_10mali e
    ON CAST(s."GEOLEVEL1" AS integer) = CAST(e."GEO1_ML" AS integer)
WHERE CAST(s."GEOLEVEL1" AS integer) IN (
    466001, 466002, 466003, 466004,
    466005, 466006, 466007, 466009
);
...
DROP TABLE IF EXISTS public.edu_birthplace_10mali;

CREATE TABLE public.edu_birthplace_10mali AS
...
-- join
DROP TABLE IF EXISTS public.geom_edu_birthplace_10mali;

CREATE TABLE public.geom_edu_birthplace_10mali AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
    ...
    e.nc_mali
FROM public.world_geolev1_2021 s
LEFT JOIN public.edu_birthplace_10mali e
