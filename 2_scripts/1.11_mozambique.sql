...
-- Project      : RHI 2026 – IPUMS Africa (Mozambique) Preprocessing
-- Script name  : 1.11_mozambique.sql
-- Last updated : 16/04/2026
-- Purpose      : Preprocess IPUMS census data (Mozambique) for statistical analysis and spatial integration.
...
CREATE TABLE public.extract_11mozambique (
  "COUNTRY"         text,
  "YEAR"            text,
  "SAMPLE"          text,
  "SERIAL"          text,
...
-- Backup
CREATE TABLE extract_11mozambique_backup AS
SELECT * FROM extract_11mozambique;

-- Master table for further analysis
CREATE TABLE master_11mozambique AS
SELECT * FROM extract_11mozambique;
...
ALTER TABLE public.master_11mozambique
ADD COLUMN IF NOT EXISTS birth_province integer;

UPDATE public.master_11mozambique
SET birth_province = CASE
    WHEN CAST("BPL1_MZ" AS integer) BETWEEN 508001 AND 508011
        THEN CAST("BPL1_MZ" AS integer)
    WHEN CAST("BPL1_MZ" AS integer) IN (508097, 508098, 508099)
        THEN NULL
    ELSE NULL
END;
...
UPDATE public.master_11mozambique
SET "FOREIGN" = CASE
    WHEN CAST("BPL1_MZ" AS integer) = 508097 THEN 1
    WHEN CAST("BPL1_MZ" AS integer) IN (508098, 508099) THEN NULL
    ELSE 0
END;
...
DELETE FROM public.master_11mozambique
...
DROP TABLE IF EXISTS public.edu_residence_11mozambique;

CREATE TABLE public.edu_residence_11mozambique AS
SELECT
    cohort,
    "GEO1_MZ",
    ...
    -- counts
    COUNT(primary_educ) AS nc_mozambique

FROM public.master_11mozambique
WHERE cohort NOT IN (1905, 1985)
GROUP BY cohort, "GEO1_MZ";


-- Join
DROP TABLE IF EXISTS public.geom_edu_residence_11mozambique;

CREATE TABLE public.geom_edu_residence_11mozambique AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
...
LEFT JOIN public.edu_residence_11mozambique e
    ON CAST(s."GEOLEVEL1" AS integer) = CAST(e."GEO1_MZ" AS integer)
WHERE CAST(s."GEOLEVEL1" AS integer) IN (
    508001, 508002, 508003, 508004, 508005,
    508006, 508007, 508008, 508009, 508010,
    508011
);
...
DROP TABLE IF EXISTS public.edu_birthplace_11mozambique;

CREATE TABLE public.edu_birthplace_11mozambique AS
...
-- join
DROP TABLE IF EXISTS public.geom_edu_birthplace_11mozambique;

CREATE TABLE public.geom_edu_birthplace_11mozambique AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
    ...
    e.nc_mozambique
FROM public.world_geolev1_2021 s
LEFT JOIN public.edu_birthplace_11mozambique e
