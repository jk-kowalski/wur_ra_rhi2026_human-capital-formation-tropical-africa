...
CREATE TABLE public.extract_12rwanda2012 (
  "COUNTRY"         text,
  "YEAR"            text,
  "SAMPLE"          text,
  "SERIAL"          text,
...
-- Backup
CREATE TABLE extract_12rwanda2012_backup AS
SELECT * FROM extract_12rwanda2012;

-- Master table for further analysis
CREATE TABLE master_12rwanda2012 AS
SELECT * FROM extract_12rwanda2012;
...
ALTER TABLE public.master_12rwanda2012
DROP COLUMN IF EXISTS birth_province;

ALTER TABLE public.master_12rwanda2012
ADD COLUMN birth_province integer;

UPDATE public.master_12rwanda2012
SET birth_province = CASE
    WHEN CAST("BPLRW2" AS integer) IN (11, 12, 13) THEN 1
    WHEN CAST("BPLRW2" AS integer) IN (21, 22, 23, 24, 25, 26, 27, 28) THEN 2
    WHEN CAST("BPLRW2" AS integer) IN (31, 32, 33, 34, 35, 36, 37) THEN 3
    WHEN CAST("BPLRW2" AS integer) IN (41, 42, 43, 44, 45) THEN 4
    WHEN CAST("BPLRW2" AS integer) IN (51, 52, 53, 54, 55, 56, 57) THEN 5
    WHEN CAST("BPLRW2" AS integer) IN (98, 99, 0) THEN NULL
    ELSE NULL
END
WHERE CAST("YEAR" AS integer) = 2012;
...
UPDATE public.master_12rwanda2012
SET "FOREIGN" = CASE
    WHEN CAST("BPLRW2" AS integer) = 98 THEN 1
    WHEN CAST("BPLRW2" AS integer) = 99 THEN NULL
    ELSE 0
END
WHERE CAST("YEAR" AS integer) = 2012;
...
DELETE FROM public.master_12rwanda2012
WHERE cohort IS NULL;

-- Restrict age range to 20-99
    -- Sample is from 2012, if you don't grab 99 as upper limit, you lose the entire 1915 cohort. 
DELETE FROM public.master_12rwanda2012
WHERE age_clean < 20
   OR age_clean > 99;
...
DROP TABLE IF EXISTS public.edu_residence_12rwanda2012;

CREATE TABLE public.edu_residence_12rwanda2012 AS
SELECT
    cohort,
    "GEO1_RW2012",
...
FROM public.master_12rwanda2012
WHERE cohort NOT IN (1905, 1985)
  AND "YEAR" = 2012
GROUP BY cohort, "GEO1_RW2012";


-- Join to 2012 Rwanda province geometry
DROP TABLE IF EXISTS public.geom_edu_residence_12rwanda2012;

CREATE TABLE public.geom_edu_residence_12rwanda2012 AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
    ...
    e.tertiary_educ,
    e.nc_rwanda
FROM public.geo1_rw2012 s
LEFT JOIN public.edu_residence_12rwanda2012 e
    ON LPAD(CAST(s."IPUM2012" AS text), 3, '0') =
       LPAD(CAST(e."GEO1_RW2012" AS text), 3, '0')
WHERE LPAD(CAST(s."IPUM2012" AS text), 3, '0') IN (
    '001', '002', '003', '004', '005'
);

-- ----------------------------------------------------------
-- 4.2 Summary by birthplace province, 2012 borders
-- ----------------------------------------------------------

DROP TABLE IF EXISTS public.edu_birthplace_12rwanda2012;

CREATE TABLE public.edu_birthplace_12rwanda2012 AS
...
-- Join to 2012 Rwanda province geometry
DROP TABLE IF EXISTS public.geom_edu_birthplace_12rwanda2012;

CREATE TABLE public.geom_edu_birthplace_12rwanda2012 AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
    ...
    e.nc_rwanda
FROM public.geo1_rw2012 s
LEFT JOIN public.edu_birthplace_12rwanda2012 e
    ON LPAD(CAST(s."IPUM2012" AS text), 3, '0') =
       LPAD(CAST(e.birth_province AS text), 3, '0')
WHERE LPAD(CAST(s."IPUM2012" AS text), 3, '0') IN (
    '001', '002', '003', '004', '005'
);