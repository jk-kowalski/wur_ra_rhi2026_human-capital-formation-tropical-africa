...
-- Project      : RHI 2026 – IPUMS Africa (Liberia) Preprocessing
-- Script name  : 1.19_liberia.sql
-- Last updated : 12/06/2026
-- Purpose      : Preprocess IPUMS census data (Liberia) for statistical analysis and spatial integration.
...
CREATE TABLE public.extract_19liberia (
  "COUNTRY"          text,
  "YEAR"             text,
  "SAMPLE"           text,
  "SERIAL"           text,
...
-- Backup
CREATE TABLE extract_19liberia_backup AS
SELECT * FROM extract_19liberia;

-- Master table for further analysis
CREATE TABLE master_19liberia AS
SELECT * FROM extract_19liberia;
...
ALTER TABLE public.master_19liberia
ADD COLUMN IF NOT EXISTS birth_province integer;

-- Recodes BPLLR to follow GEO1_LR
UPDATE public.master_19liberia
SET birth_province = CASE

    -- Grand Cape Mount
    WHEN CAST("BPLLR" AS integer) = 12 THEN 430012

    -- Gbarpolu, Lofa
    WHEN CAST("BPLLR" AS integer) IN (21, 45) THEN 430021

    -- Grand Gedeh, Grand Kru, Maryland, River Gee, Rivercess, Sinoe
    WHEN CAST("BPLLR" AS integer) IN (15, 18, 27, 36, 39, 42) THEN 430027

    -- Bomi, Bong, Grand Bassa, Margibi, Montserrado
    WHEN CAST("BPLLR" AS integer) IN (3, 6, 9, 24, 30) THEN 430030

    -- Nimba
    WHEN CAST("BPLLR" AS integer) = 33 THEN 430033

    -- Foreign country / Unknown
    WHEN CAST("BPLLR" AS integer) IN (98, 99) THEN NULL
...
UPDATE public.master_19liberia
SET "FOREIGN" = CASE
    WHEN CAST("BPLLR" AS integer) = 98 THEN 1
    WHEN CAST("BPLLR" AS integer) = 99 THEN NULL
    ELSE 0
END;
...
DELETE FROM public.master_19liberia
...
DROP TABLE IF EXISTS public.edu_residence_19liberia;

CREATE TABLE public.edu_residence_19liberia AS
SELECT
    cohort,
    "GEO1_LR",
    ...
    COUNT(primary_educ) AS nc_liberia

FROM public.master_19liberia
WHERE cohort NOT IN (1905, 1985)
GROUP BY cohort, "GEO1_LR";


-- Join
DROP TABLE IF EXISTS public.geom_edu_residence_19liberia;

CREATE TABLE public.geom_edu_residence_19liberia AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
...
LEFT JOIN public.edu_residence_19liberia e
    ON CAST(s."GEOLEVEL1" AS integer) = CAST(e."GEO1_LR" AS integer)
WHERE CAST(s."GEOLEVEL1" AS integer) IN (
    430012, 430021, 430027, 430030, 430033
);
...
DROP TABLE IF EXISTS public.edu_birthplace_19liberia;

CREATE TABLE public.edu_birthplace_19liberia AS
...
-- Join
DROP TABLE IF EXISTS public.geom_edu_birthplace_19liberia;

CREATE TABLE public.geom_edu_birthplace_19liberia AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
    ...
    e.nc_liberia
FROM public.world_geolev1_2021 s
LEFT JOIN public.edu_birthplace_19liberia e
