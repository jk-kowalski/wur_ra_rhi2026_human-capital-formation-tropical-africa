...
CREATE TABLE public.extract_20cameroon (
  "COUNTRY"          text,
  "YEAR"             text,
  "SAMPLE"           text,
  "SERIAL"           text,
...
-- Backup
CREATE TABLE extract_20cameroon_backup AS
SELECT * FROM extract_20cameroon;

-- Master table for further analysis
CREATE TABLE master_20cameroon AS
SELECT * FROM extract_20cameroon;
...
ALTER TABLE public.master_20cameroon
...
UPDATE public.master_20cameroon
SET "FOREIGN" = CASE
    WHEN CAST("BPL1_CM" AS integer) = 120097 THEN 1
    WHEN CAST("BPL1_CM" AS integer) = 120098 THEN NULL
    ELSE 0
END;
...
DELETE FROM public.master_20cameroon
...
DROP TABLE IF EXISTS public.edu_residence_20cameroon;

CREATE TABLE public.edu_residence_20cameroon AS
SELECT
    cohort,
    "GEO1_CM",
...
GROUP BY cohort, "GEO1_CM";


-- Join
DROP TABLE IF EXISTS public.geom_edu_residence_20cameroon;

CREATE TABLE public.geom_edu_residence_20cameroon AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
...
LEFT JOIN public.edu_residence_20cameroon e
    ON CAST(s."GEOLEVEL1" AS integer) = CAST(e."GEO1_CM" AS integer)
