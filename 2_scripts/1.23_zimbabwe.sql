...
-- Project      : RHI 2026 – IPUMS Africa (Cameroon) Preprocessing
-- Script name  : 1.20_cameroon.sql
-- Last updated : 12/06/2026
-- Purpose      : Preprocess IPUMS census data (Cameroon) for statistical analysis and spatial integration.
...
-- Description:
  ...
  -- Source data:
    -- IPUMS International: SCameroon Census Microdata (2012, 2002, 1988)
...
CREATE TABLE public.extract_23zimbabwe (
  "COUNTRY"              text,
  "YEAR"                 text,
  "SAMPLE"               text,
  "SERIAL"               text,
  ...
  "ZW2012A_BPL"          text,
  "ZW2012A_BPL2"         text,
  "ZW2012A_USUALRES"     text,
  "ZW2012A_USUALRES2"    text,
  "ZW2012A_RES10YR"      text,
  "ZW2012A_RES10YR3"     text,
  "ZW2012A_ETHNIC"       text,
  "ZW2012A_CITIZEN"      text,
  "ZW2012A_SCHEVER"      text,
  "ZW2012A_EDLEVEL"      text,
  "ZW2012A_EDATTAIN"     text,
  "ZW2012A_SCHOOL"       text,
  "ZW2012A_SCHYR"        text,
  "ZW2012A_SCHLEVEL"     text,
  "ZW2012A_SCHGRADE"     text,
  "ZW2012A_FIELD"        text
);
...
-- Backup
CREATE TABLE extract_23zimbabwe_backup AS
SELECT * FROM extract_23zimbabwe;

-- Master table for further analysis
CREATE TABLE master_23zimbabwe AS
SELECT * FROM extract_23zimbabwe;
...
ALTER TABLE public.master_23zimbabwe
ADD COLUMN IF NOT EXISTS birth_province integer;

-- Recodes BPLLR to follow GEO1_LR
UPDATE public.master_23zimbabwe
SET birth_province = CASE

    -- Bulawayo
    WHEN CAST("ZW2012A_BPL" AS integer) = 2 THEN 716000

    -- Manicaland
    WHEN CAST("ZW2012A_BPL" AS integer) IN (10, 12) THEN 716001

    -- Mashonaland Central
    WHEN CAST("ZW2012A_BPL" AS integer) IN (20, 22) THEN 716002

    -- Mashonaland East
    WHEN CAST("ZW2012A_BPL" AS integer) IN (30, 32) THEN 716003

    -- Mashonaland West
    WHEN CAST("ZW2012A_BPL" AS integer) IN (40, 42) THEN 716004

    -- Matabeleland North
    WHEN CAST("ZW2012A_BPL" AS integer) IN (50, 52) THEN 716005

    -- Matabeleland South
    WHEN CAST("ZW2012A_BPL" AS integer) IN (60, 62) THEN 716006

    -- Midlands
    WHEN CAST("ZW2012A_BPL" AS integer) IN (70, 72) THEN 716007

    -- Masvingo
    WHEN CAST("ZW2012A_BPL" AS integer) IN (80, 82) THEN 716008

    -- Harare
    WHEN CAST("ZW2012A_BPL" AS integer) IN (90, 92) THEN 716009

    -- Foreign / Unknown
    WHEN CAST("ZW2012A_BPL" AS integer) IN (94, 95, 98) THEN NULL
...
UPDATE public.master_23zimbabwe
SET "FOREIGN" = CASE
    WHEN CAST("ZW2012A_BPL" AS integer) IN (94, 95) THEN 1
    WHEN CAST("ZW2012A_BPL" AS integer) = 98 THEN NULL
    ELSE 0
END;
...
DELETE FROM public.master_23zimbabwe
...
DROP TABLE IF EXISTS public.edu_residence_23zimbabwe;

CREATE TABLE public.edu_residence_23zimbabwe AS
SELECT
    cohort,
    "GEO1_ZW",
    ...
    COUNT(primary_educ) AS nc_zimbabwe

FROM public.master_23zimbabwe
WHERE cohort NOT IN (1905, 1985)
GROUP BY cohort, "GEO1_ZW";


-- Join
DROP TABLE IF EXISTS public.geom_edu_residence_23zimbabwe;

CREATE TABLE public.geom_edu_residence_23zimbabwe AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
...
LEFT JOIN public.edu_residence_23zimbabwe e
    ON CAST(s."GEOLEVEL1" AS integer) = CAST(e."GEO1_ZW" AS integer)
...
DROP TABLE IF EXISTS public.edu_birthplace_23zimbabwe;

CREATE TABLE public.edu_birthplace_23zimbabwe AS
...
-- Join
DROP TABLE IF EXISTS public.geom_edu_birthplace_23zimbabwe;

CREATE TABLE public.geom_edu_birthplace_23zimbabwe AS
SELECT
    s.*,
    e.cohort,
    e.primary_educ,
    ...
    e.nc_zimbabwe
FROM public.world_geolev1_2021 s
LEFT JOIN public.edu_birthplace_23zimbabwe e
